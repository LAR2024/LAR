    ...
    def new_main(self):
        ...
        while not self.turtle.is_shutting_down():
            ...
            if len(pairs) > 0:
                ...
                if positon == 'space' and ((p_z > min_fi and math.atan(p_x/p_z) > min_dist) or last_target == (None, None, None)):
                    fi = math.atan(p_x / p_z)
                    dist = p_z
                    last_target = (p_x,p_y,p_z)
                elif p_z <= min_dist and math.atan(p_x/p_z) <= min_fi and positon == 'space':
