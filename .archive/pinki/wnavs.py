...
def display(location,color,marker_type='column',pointer=None):
    if marker_type=='column' or marker_type=='target':
        cv2.drawMarker(img, origin+location, color, cv2.MARKER_SQUARE, 10, 1)
    if marker_type=='robot' or marker_type=='target':
        cv2.drawMarker(img, origin+location, color, cv2.MARKER_TILTED_CROSS, 10, 1)
    if marker_type=='highlight':
        cv2.drawMarker(img, origin+location, color, cv2.MARKER_SQUARE, 15, 1)
    if pointer is not None:
        cv2.line(img,origin+location,origin+location+pointer, color,1)
...
def displayConnection(col1,col2,is_flag):
    if SHOW_ALL_CONNECTIONS or (SHOW_ACTIVE_CONNECTIONS and is_flag):
        ...
        cv2.line(img,origin+col1['pos'],origin+col2['pos'],connection_color,1)

def getTargetsFromFlag(col1,col2):
    targets=[]
    flag_center=col2['pos']+(col1['pos']-col2['pos'])//2
    connection_direction=np.array(((col1['pos']-col2['pos'])/np.linalg.norm(col1['pos']-col2['pos']))*TARGET_FROM_CENTER,dtype=np.int64)

    relative_target=np.matmul([[0,1],[-1,0]],connection_direction,dtype=np.int64)
...
def getTargets(columns):
    ...
    for col1 in columns:
        ...
        if SHOW_TRACKERS:
            cv2.line(img,origin+col1['pos'],origin,color,1)
            if SHOW_TRACKERS_DIST:
                cv2.putText(img, "%.2f"%(np.linalg.norm(col1['pos'])) , (origin+(col1['pos'])//2+10) , FONT, 0.4,color, 1)
...
def nameTargets(targets,old_names):
    ...
    for target in targets:
        ...
        for name in old_names.keys():
            if (np.isclose(target,old_names[name],0.05,10)).all():
                names[name]=target
                named=True
                break
        if not named:
...
def displayTargetNames(names):
    for name in names.keys():
        cv2.putText(img, name , (origin+names[name][0]) , FONT, 1,(255,255,255), 1)
