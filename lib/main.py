...
class Control:
    ...
    def loop(self):
        while not self.turtle.is_shutting_down():
            ...
            if image is not None and pc is not None:
                process_imgs.get_pairs_from_cams(image,pc)

    def new_main(self):
        last_target = (None, None, None)

        last_m = (None, None, None)
        last_t = (None, None, None)
        ...
        while not self.turtle.is_shutting_down():
            pairs = self.get_pairs_from_cams()
            ...
            if len(pairs) > 0:
                ...
                if (p_z <= min_dist and math.atan(p_x / p_z) <= min_fi and positon == 'space'):
                    positon = 'purple-blue'
                elif t_z <= min_dist and math.atan(t_x / t_z) <= min_fi and positon == 'purple-blue':
                    positon = 'blue-yellow'
                elif math.atan(m_x / m_z) <= min_fi and positon == 'blue-yellow':
                    positon = 'finding'
                ...
                if positon == 'space':
                    ...
                    dist = p_z
                    last_target = last_p
                elif positon == 'purple-blue':
                    fi = math.atan(t_x / t_z)
                    dist = t_z
                    last_target = last_t
                elif positon == 'blue-yellow':
                    fi = math.atan(m_x / m_z)
                    dist = 0
                    last_target = last_m
                elif positon == 'finding':
                    fi = 0
                    dist = 0
                ...
                print(fi, dist, 'cam')
                self.move_2_target(fi, dist)


            elif last_target != (None, None, None):
                x, y, z = last_target
                ...
                r_fi = o_fi - math.atan(r_x / r_z)

                print(r_fi, r_z, 'odo')
                self.move_2_target(r_fi, r_z)

    def get_pairs_from_cams(self):
        image = self.turtle.get_rgb_image()
        pc = self.turtle.get_point_cloud()
        ...
        for index1, pole1 in enumerate(poles.poles):
            for index2, pole2 in enumerate(poles.poles):
                if pole1.pos[0] is not None and pole2.pos[0] is not None:
                    if index1 != index2:
                        diff = ((pole1.pos[0] - pole2.pos[0]) ** 2 + (pole1.pos[1] - pole2.pos[1]) ** 2 + (
                                    pole1.pos[2] - pole2.pos[2]) ** 2) ** 0.5
                        if diff < 0.2 and ((pole1.color == 'r' and pole2.color == 'b') or (
                                pole1.color == 'b' and pole2.color == 'r') or (
                                                   pole1.color == 'g' and pole2.color == 'g')):
